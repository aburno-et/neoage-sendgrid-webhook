...
async function pollEmailLogsForAccount(account) {
  ...
  // --- SendGrid Logs API hard limit: lower bound must be within last 30 days ---
  ...
  // Overlap to avoid missing edge events (dedupe/upsert handles duplicates)
  ...
  // Use the same window-splitting backfill logic so we don't silently cap at 1000.
  const r = await processWindowRecursive(account, sinceOverlap, until);
  ...
  return {
    ...
    since: sinceStr,
    until: untilStr,
    foundMessages: r.foundMessages || 0,
    hydrated: r.hydrated || 0,
...
async function refreshAccount(account, days = 30) {
  // Refresh/re-hydrate the last N days by walking fixed windows.
  // (SendGrid only allows up to 30 days of lookback via /v3/logs.)
  const now = new Date();
  ...
  let totals = { foundMessages: 0, hydrated: 0, inserted: 0, cappedWindows: 0 };
  ...
  while (cursor < now) {
    ...
    const r = await processWindowRecursive(account, cursor, windowEnd);

    totals.foundMessages += r.foundMessages || 0;
    totals.hydrated += r.hydrated || 0;
    totals.inserted += r.inserted || 0;
    if (r.capped) totals.cappedWindows += 1;
  ...
  // Optionally align poll cursor with "now" after a refresh
  await setLastSeen(account.id, now);

  return { account: account.id, days, ...totals };
}
...
async function refreshAllAccounts(days = 30) {
  ...
  for (const acct of SENDGRID_ACCOUNTS) {
    try {
      results.push(await refreshAccount(acct, days));
    } catch (err) {
      console.error(`Refresh failed for ${acct.id}:`, err);
