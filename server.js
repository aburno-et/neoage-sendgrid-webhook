...
async function upsertGptDay(domain, dayStr, item) {
  // dayStr must be 'YYYY-MM-DD'
  await pool.query(
    `
    ...
    set
      ...
      fetched_at = now()
    `,
    [domain, dayStr, JSON.stringify(item)]
  );
}
...
app.post("/admin/poll", async (req, res) => {
  ...
  // Hard guard: never query earlier than ~30 days (SendGrid constraint)
    ...
    const MAX_RUNTIME_MS = Math.min(600_000, Number(process.env.POLL_MAX_RUNTIME_MS || 240_000));
