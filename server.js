...
const MIN_WINDOW_MS = 60 * 1000; // 1 minute minimum window when splitting
...
async function runBackfillAll(days, note) {
  const d = Math.min(Number(days || 30), 30);
...
async function cleanupOldData() {
  const r = await pool.query(`delete from sendgrid_events where event_ts < now() - interval '90 days'`);
  return r.rowCount || 0;
}
...
app.post("/admin/poll", async (req, res) => {
  ...
  for (const acct of SENDGRID_ACCOUNTS) {
    try {
      const since = await getLastSeen(acct.id);
